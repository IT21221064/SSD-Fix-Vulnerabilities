  ...
  try {
    ...
    if (
      ...
      return response.status(400).json({
        message:
          "Send all required fields: employeeName, employeeEmail, employeeMobile, employeeAddress, employeeRole, createdOn, password",
      });
    }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    response
      .status(200)
      .json({ message: "Employee profile deleted successfully" });
  } catch (error) {
    console.error(error);
    response.status(500).json({ message: "An error occurred on the server" });
  }
