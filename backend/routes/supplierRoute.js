  ...
  try {
    ...
    if (!supplierid || !name || !address || !contact || !email) {
      return response.status(400).send({
        message:
          "Please provide all required fields: SupplierID, Name, Address, ContactNo, Email",
      });
    }
    ...
    return response.status(201).json(createSupplier);
  } catch (error) {
...
router.put("/:id", async (request, response) => {
  try {
    const { id } = request.params;
    const updateSupplier = await supplier.findByIdAndUpdate(id, request.body, {
      new: true,
    });
    if (!updateSupplier) {
      return response.status(404).send({ message: "Supplier not found" });
    }
    return response.status(200).json(updateSupplier);
  } catch (error) {
    console.error(error.message);
    response.status(500).send({ message: error.message });
  }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "Supplier deleted successfully!" });
  } catch (error) {
    console.error(error.message);
    response.status(500).send({ message: error.message });
  }
