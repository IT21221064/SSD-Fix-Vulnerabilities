  ...
  try {
    ...
    if (!wasteid || !teatype || !batchid || !quantity) {
      return response.status(400).send({
        message: "Please provide wasteid, teatype, batchid, and quantity",
      });
    }
    ...
    if (inventoryItem.quantity < quantity || quantity < 1) {
      return response.status(400).send({
        message: "Insufficient quantity in inventory or invalid waste quantity",
      });
    }
    ...
    return response.status(201).json(createdWasteEntry);
  } catch (error) {
...
router.put("/:id", async (request, response) => {
  try {
    const { id } = request.params;
    const updatedWasteEntry = await waste.findByIdAndUpdate(id, request.body, {
      new: true,
    });
    return response.status(200).json(updatedWasteEntry);
  } catch (error) {
    console.log(error.message);
    response.status(500).send({ message: error.message });
  }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "Waste entry deleted successfully!" });
  } catch (error) {
    console.log(error.message);
    response.status(500).send({ message: error.message });
  }
