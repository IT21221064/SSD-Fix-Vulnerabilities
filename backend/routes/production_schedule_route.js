  ...
  try {
    if (
      ...
      return response.status(400).send({
        message:
          "send all required fields: Schedule_no, Production_date, Quantity, Machine_assignment, shift_information",
      });
    }
    const new_schedule = {
...
router.put("/:id", async (request, response) => {
  try {
    ...
    if (!result) {
      return response
        .status(404)
        .json({ message: "production schedule not found" });
    }
    return response
      .status(200)
      .send({ message: "production shedule update successfully" });
  } catch (error) {
    console.log(error.message);
    return response.status(500).send({ message: error.message });
  }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "production shedule delete successfully" });
  } catch (error) {
    console.log(error.message);
    return response.status(500).send({ message: error.message });
  }
