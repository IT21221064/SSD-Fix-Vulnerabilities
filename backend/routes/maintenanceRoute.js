  ...
  try {
    if (
      ...
      return response.status(400).send({
        message:
          "send all required fields: machineNumber, machineName, description, maintenanceDate, frequencyInDays",
      });
    }
...
router.put("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "maintenance schedule update successfully" });
  } catch (error) {
    console.log(error.message);
    return response.status(500).send({ message: error.message });
  }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "maintenance schedule deleted successfully" });
  } catch (error) {
    console.log(error.message);
    return response.status(500).send({ message: error.message });
  }
