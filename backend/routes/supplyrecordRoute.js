  ...
  try {
    ...
    if (!supplier || !date || !quantity || !unitPrice) {
      return response
        ...
        .send({
          message:
            "Send all required fields: supplier, date, quantity, unitPrice",
        });
    }
...
router.put("/:id", async (request, response) => {
  try {
    const { id } = request.params;
    const updateRecord = await SupplyRecord.findByIdAndUpdate(
      id,
      request.body,
      { new: true }
    );
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "Supply record deleted successfully" });
  } catch (error) {
    console.error(error.message);
    response.status(500).send({ message: error.message });
  }
...
router.put("/changeStatus/:id", async (request, response) => {
  try {
    ...
    const updateSupply = await SupplyRecord.findByIdAndUpdate(
      id,
      { status },
      { new: true }
    );
    ...
    return response.status(200).json(updateSupply);
  } catch (error) {
    console.error(error.message);
    response.status(500).send({ message: error.message });
  }
