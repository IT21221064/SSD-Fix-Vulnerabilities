  ...
  try {
    ...
    if (!batchid || !category || !inventorynumber || !quantity) {
      return response.status(400).send({
        message:
          "Please provide all required fields: batchid, category, inventorynumber, quantity",
      });
    }
    ...
    return response.status(201).json(createdInventoryEntry);
  } catch (error) {
...
router.put("/:id", async (request, response) => {
  try {
    ...
    return response.status(200).json(updatedInventoryEntry);
  } catch (error) {
    console.log(error.message);
    response.status(500).send({ message: error.message });
  }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "Inventory entry deleted successfully!" });
  } catch (error) {
    console.log(error.message);
    response.status(500).send({ message: error.message });
  }
