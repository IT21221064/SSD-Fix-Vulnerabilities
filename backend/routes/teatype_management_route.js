  ...
  try {
    if (
      ...
      return response.status(400).send({
        message:
          "send all required fields: Schedule_no, black_tea, green_tea, oolong_tea, white_tea, tea_wastage, status",
      });
    }
    ...
    return response.status(201).json(teatype);
  } catch (error) {
...
router.put("/:id", async (request, response) => {
  try {
    ...
    if (!result) {
      return response
        .status(404)
        .json({ message: "Tea type schedule not found" });
    }
    return response
      .status(200)
      .send({ message: "Tea type schedule updated successfully" });
  } catch (error) {
    console.log(error.message);
    return response.status(500).send({ message: error.message });
  }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .send({ message: "Tea type schedule deleted successfully" });
  } catch (error) {
    console.log(error.message);
    return response.status(500).send({ message: error.message });
  }
...
router.put("/:id/update-status", async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .json({
        message: "Tea type schedule status updated successfully",
        data: teatype,
      });
  } catch (error) {
