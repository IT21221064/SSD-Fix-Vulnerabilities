...
const allowedOrigins = ["http://localhost:5173"]; // Add only the trusted origins

const corsOptions = {
  origin: function (origin, callback) {
    // If no origin or the origin is in the allowed list
  ...
  methods: ["GET", "POST", "PUT", "DELETE"], // Restrict to the methods you want to allow
  allowedHeaders: ["Content-Type", "Authorization"], // Specify the headers allowed
  credentials: true, // Set to true if cookies are required for cross-origin requests
  optionsSuccessStatus: 204, // Use 204 for successful OPTIONS preflight requests
};
...
app.use(
  "/uploads",
  express.static(
    "C:\\Users\\IMASHA\\Documents\\GitHub\\TEA_FACTORY_MANAGEMENT_SYSTEM_Year_2_Semester_2_Project\\backend\\uploads"
  )
);
